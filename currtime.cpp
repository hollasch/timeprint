...
#define formatSize 2048              // Maximum Size of Format String

int main (int argc, char *argv[])
{
    time_t     long_time;
    time_t     offset_base = -1;           // Offset Base Time
    char      *offset_base_file = NULL;    // Offset Base File
    struct tm *currtime;
    char      *fmtptr;                     // Pointer into Format String
    char      *zone = 0;                   // Time Zone;
    char       escchar = '%';              // Escape Character
    int        i;                          // Loop Variable
    ...
    for (i=1;  i < argc;  ++i)
    {
        ...
        {
            if (format[0] == 0)
                strcpy (format, argptr);
            else
            {
                strcat (format, " ");
                strcat (format, argptr);
            }
        }
        else
        {
            char optchar = argv[i][1];     // Option Character
            ...
            // Point argptr to the contents of the switch.  This may be
            // immediately following the option character, or it may be the
            // next token on the command line.
            ...
            switch (optchar)
            {
                default:
                    fprintf (
                        stderr, "Error: Unrecognized option (-%c).\n", optchar);
                    return -1;
            ...
            // Currently, all options take an argument, so flag an error if
    ...
    // If no format string was specified on the command line, fetch it from
    // the TIMEFORMAT environment variable.  If not available there, then
    ...
    if (format[0] == 0)
    {
        ...
        // Any alternate escape character (through the -e switch) is ignored
        // if the format is not specified on the command line.

        escchar = '%';

        envFormat = getenv ("TIMEFORMAT");

        if (envFormat == 0)
            envFormat = "%#c";

        strcpy (format, envFormat);
    }

    // If an alternate time zone was specified, then we need to set the TZ
    // environment variable.  Kludgey, but I couldn't find another way.

    if (zone)
    {
        char *buff = new char [1 + strlen("TZ=") + strlen(zone)];
    ...
    // If an offset base file was specified, get the modification time from the
    // file.
    ...
    if (offset_base_file)
    {
        ...
        if (0 != _stat(offset_base_file, &stat))
        {
    ...
    if (offset_base_file)
    {
        if (long_time < offset_base)
        {
            fprintf (stderr, "currtime: Time zone error: is your environment "
                             "variable TZ set incorrectly?\n");
            return -1;
        }

        long_time -= offset_base;
        currtime = gmtime (&long_time);
    }
    else
    {
        currtime = localtime (&long_time);
    }

    // Now scan through the format string, emitting expanded characters along
    // the way.
    ...
    for (fmtptr=format;  *fmtptr;  ++fmtptr)
    {
        ...
        // Handle \-sequences, unless \ is the alternate escape character.
        ...
        if ((*fmtptr == '\\') && (escchar != '\\'))
        {
            ...
            switch (*fmtptr)
            {
                    // Unrecognized \-sequences resolve to the escaped
                    // character.
        ...
        else if (*fmtptr == escchar)
        {
            ...
            switch (*fmtptr)
            {
                case 'D':   // Total Elapsed Whole Days
                    printf ("%lu", long_time / (24*60*60));
                    break;

                case 'R':  // Total Elapsed Whole Hours
                    printf ("%lu", long_time / (60*60));
                    break;
